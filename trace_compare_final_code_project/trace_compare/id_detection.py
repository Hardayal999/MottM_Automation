...
def _value_score(ws, col, start_row, sample_rows=300):
    ...
    if total == 0:
        return 0
    return int((code_like / total) * 8) + min(4, blank_or_repeated // 20)


def detect_id_columns(new_ws, old_ws, header_rows=1):
    """
    Best-effort ID column detection. Manual --id-columns is still recommended for production.
    """
    ...
    filtered = []
    for col in cols:
        # ID columns in traceability files usually have descriptive columns between them.
