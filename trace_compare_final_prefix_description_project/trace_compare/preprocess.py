...
def build_merged_lookup(ws, columns: Iterable[int]) -> Dict[Tuple[int, int], Tuple[int, int]]:
    ...
    the merged range's top-left value. We use this for ID columns and detail
    columns so merged descriptions are compared correctly too.
    """
...
def sheet_to_records(ws, id_columns: List[int], max_col: int, config: CompareConfig) -> List[RowRecord]:
    # Important: resolve actual merged ranges for every column, not just ID columns.
    # This lets description/detail text in merged cells be compared at the correct hierarchy level.
    merged_lookup = build_merged_lookup(ws, range(1, max_col + 1))
